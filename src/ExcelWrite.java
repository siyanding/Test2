...
public class ExcelWrite {
			...
			if(titleRow!=null){
				for(int rowId=0;rowId<mapList.size();rowId++){
					...
					for (short columnIndex = 0; columnIndex < columnCount; columnIndex++) {  //遍历表头
						String mapKey = titleRow.getCell(columnIndex).toString().trim();//titleRow.getCell(columnIndex).toString().trim().toString().trim()
	...
	public static String[] readMethod(){
		...
		int i = 0;
		try{
			BufferedReader in=new BufferedReader(new FileReader("D:/read.txt"));
			line = in.readLine();
